...
def file_to_vectors(file_name,
    ...
    # stft_matric = librosa.core.stft(y, n_fft=n_fft, hop_length=hop_length
    #                                 , win_length=1024, window='hann')
    # STFT = np.abs(stft_matric)**2     
    # STFT = librosa.core.power_to_db(STFT,ref=np.min)
    # STFT = STFT[:-1,:]
    ### print(log_mel_spectrogram.shape)
    ######calculate total vector size
    # n_vectors = len(STFT[0, :]) - n_frames + 1
    ...
    for t in range(n_frames):
        # vectors[:, n_mels * t : n_mels * (t + 1)] = STFT[:, t : t + n_vectors].T
